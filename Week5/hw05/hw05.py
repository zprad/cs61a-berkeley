...
def balanced(m):
    ...
    if is_weight(m):
      ...
      assert is_mobile(m), "must compute balanced of a mobile or a weight"
...
def totals_tree(m):
    ...
    if is_weight(m):
      ...
      assert is_mobile(m), "must compute totals tree of a mobile or a weight"
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    incorrect_passwords = []
    def withdraw(amount, type_password):
      ...
      if type_password == password:
        ...
        incorrect_passwords.append(type_password)
...
def make_joint(withdraw, old_password, new_password):
    ...
    confirm_password_result = withdraw(0, old_password)
    if type(confirm_password_result) == str:
...
def generate_paths(t, x):
    ...
    "*** YOUR CODE HERE ***"
    path_list = []
    def paths(t, base_path):
      nonlocal path_list
      path = base_path + [label(t)]
      if label(t) == x:
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) * lower_bound(y) > 0, 'can not divide by an interval that spans zero'
...
def join_link(s, word):
  if s == empty:
    return ''
  elif rest(s) == empty:
    return str(first(s))
  else:
    return str(first(s)) + word + join_link(rest(s), word)
...
def print_partitions(n, m):
  linked_lists = partitions(n, m)
  strings = apply_to_all_link(lambda s: join_link(s, ' + '), linked_lists)
