...
def put_piece(board, max_rows, column, player):
    ...
    while current_row >= 0:
        if get_piece(board, current_row, column) == '-':
            board[current_row][column] = player
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert row >= 0 and row < max_rows, 'Row is out of bounds'
    assert num_connect > 0 and num_connect <= max_cols and num_connect <= max_rows, 'Num_connect is out of bounds'
    counts, col = 0, 0
    while col < max_cols:
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert col >= 0 and col < max_cols, 'Col is out of bounds'
    assert num_connect > 0 and num_connect <= max_cols and num_connect <= max_rows, 'Num_connect is out of bounds'
    counts, row = 0, 0
    while row < max_rows:
        if get_piece(board, row, col) == player:
