...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda expr_item: scheme_eval(expr_item, env))
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
             raise SchemeError('formals: ${0} is not corresponding to vals: ${1}'.format(formals, vals))
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong number of arguments were passed when call ${0}'.format(self.name))
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    def eval_binding(binding, env):
        check_form(binding, 2, 2)
        return scheme_eval(binding.second.first, env)
    check_form(bindings, 1)
    formals = bindings.map(lambda binding: binding.first)
    check_formals(formals)
    vals = bindings.map(lambda binding: eval_binding(binding, env))
