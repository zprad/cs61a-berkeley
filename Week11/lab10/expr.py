...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError(self.string + ' is not defined')
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        params = []
        for opt in self.operands:
            params.append(opt.eval(env))
        return operator_val.apply(params)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        params = {}
        for ele in zip(self.parameters, arguments):
            params[ele[0]] = ele[1]
